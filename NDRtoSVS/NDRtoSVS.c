// NDRtoX.c      
//    
// Converts .ndr file into .lsn or .hsn file v1.2 or .h 
...
// 06.2024: writes SN matrix specification in .h file
// 07.2024: writes SN pin-map for Arduino in .h file
//
// Usage: NDRtoX file1.ndr file2.lsn
//        NDRtoX file1.ndr file2.hsn
//
...
static int n, m, maxn, maxm; /* net size: trs, pls, labels */
...
enum in_signal_level{RISING=1,FALLING,CHANGE};
enum out_signal_level{LOW=1,HIGH};
static int *pl, *plp, mi, mo, ml, max_pl; /* pl labels: plp < 0 - in, plp > 0 - out */

static char HSN_prefix[]="{*HSN("; // *HSN()
static int HSN_prefix_length=6;

static char IN_prefix[]="{*IN("; // *IN(PIN,RISING/FALLING/CHANGE)
static int IN_prefix_length=5;

static char OUT_prefix[]="{*OUT("; // *OUT(PIN,LOW/HIGH)
static int OUT_prefix_length=6;
...
//void ExpandPLI()
//{
//  int *new_pli, *new_plipn;
//
//  if( l_pi >= maxl_pi - 2 ) 
//  {
//    max_pli+=lDELTA;
//    new_pli = (int*) realloc( pli, max_pli * sizeof(int) );
//    new_plipn = (int*) realloc( plipn, maxl_p * sizeof(int) );
//    if( new_pli==NULL || new_plipn==NULL ) 
//      { printf( "*** not enough memory (ExpandTL)\n" ); exit(3); }
//    else 
//      { pli=new_pli; plipn=new_plipn; }
//  }
//  
//} /* ExpandPLI */
...
//  {
//    maxl_pli+=lDELTA;
//    newpli = (int*) realloc( pli, maxl * sizeof(int) );
//    newplipn = (int*) realloc( plipn, maxl_p * sizeof(int) );
//    if( newpli==NULL || newplipn==NULL ) 
//      { printf( "*** not enough memory (ExpandTL)\n" ); exit(3); }
//    else 
//      { pli=newpli; plipn=newplipn; }
//  }
//  
//} /* ExpandPLO */
...
void ReadNDR( FILE * f )
{
 ...
 while( ! feof( f ) )
 {
   ...
   switch( str[i++] )
   {
	...
	// tuta
	SwallowSpace( str, &i );
	while( ! IsSpace( str,i) && i<len )i++; /* marking */
	SwallowSpace( str, &i );
	while( ! IsSpace( str,i) && i<len )i++; /* eft */
	
	SwallowSpace( str, &i );
	ii=fnames;								/* check input/output place label */
	GetName( &i, &fnames );
//printf("%d %s\n",m,names+ii);
	if(memcmp(IN_prefix,names+ii,IN_prefix_length)==0)
	{
//printf("%d %s\n",m,names+ii);
	  // ExpandPL();
       pl[ ++ml ]=ii+IN_prefix_length; 
	   plp[ ml ]=-m;
       names[fnames-3]='\0';
       fnames-=2;	
	   mi++;	           
	} else 
	if(memcmp(OUT_prefix,names+ii,OUT_prefix_length)==0)
	{
//printf("%d %s\n",m,names+ii);
	  // ExpandPL();
       pl[ ++ml ]=ii+OUT_prefix_length; 
	   plp[ ml ]=m;
       names[fnames-3]='\0';
       fnames-=2;
	   mo++;	   
...
void WriteLSVS_h( FILE * f )
{
  int p, t, i, empty;
  int * b, *d, * t_neib_count;
  
  b=malloc(MATRIX_SIZE(m,n,int));
  d=malloc(MATRIX_SIZE(m,n,int));
  t_neib_count=malloc(n*sizeof(int));
  if( b==NULL || d==NULL || t_neib_count==NULL ) {
  	printf("*** error: no space for b, d, t_neib_count\n");
  	exit(-2);
  }
  memset(b,0,MATRIX_SIZE(m,n,int));
  memset(d,0,MATRIX_SIZE(m,n,int));
  memset(t_neib_count,0,n*sizeof(int));
  for( i=0; i<fapt; i++ )
    MELT(b,(aptp[i]-1),(aptt[i]-1),m,n)=(aptw[i]>0)?(-aptw[i]-1):-1;
  for( i=0; i<fatp; i++ ) {
  	// if(MELT(c,(atpp[i]-1),(atpt[i]-1),m,n) < 0) { printf("*** error: self-loop detected\n"); exit(6);} else 
	MELT(d,(atpp[i]-1),(atpt[i]-1),m,n)=atpw[i];
  }
     
  fprintf( f, "// PTN in the form of SVS obtained from NDR\n\n" );
  
  fprintf( f, "#define M %d // number of places \n"
"#define N %d // number of transitions\n\n", m, n );

  fprintf( f, "unsigned short mu[] = {" );
  // loop on places, print mu  
  for( p=1; p<=m; p++ )
  {
  	fprintf( f, "%d", mu[p] );
  	fprintf( f, "%c",(p<m)?',':'}');
  }
  fprintf( f, "; // initial place marking\n\n" );
  
  fprintf( f, "// incidental places of transitions - semaphore operations\n" );
  fprintf( f, "// struct sembuf {\n"
...
"// };\n" );
  ...
  for( t=0; t<n; t++ )
  {
    fprintf( f, "struct sembuf t%d[] = {", t );
    // loop on places
    empty=1;
    for(p=0; p<m; p++) {
      if(MELT(b,p,t,m,n)<0) { // p->t
        t_neib_count[t]++;
        fprintf( f, "%c",(empty)?' ':',' );
      	fprintf( f, "{%d,%d,%d}", p,MELT(b,p,t,m,n)+1,0); 
      	empty=0;
      }
    }  
    for(p=0; p<m; p++) {
      if(MELT(d,p,t,m,n)>0) { // t->p
	t_neib_count[t]++;
	fprintf( f, "%c",(empty)?' ':',' );
	fprintf( f, "{%d,%d,%d}", p,MELT(d,p,t,m,n),0);
	empty=0;
      } 
    }
    fprintf( f, " };\n" );
  ...
  fprintf( f, "int t_pls_num [] = {" ); 
  // loop on transitions
  for( t=0; t<n; t++ )
  {
    fprintf( f, "%d", t_neib_count[t] );
    fprintf( f, "%c",(t<n-1)?',':'}' );
  }
  fprintf( f, ";\n" );
  fprintf( f, "int t_arg[N]; // memory for thread arguments\n\n" );
  
  fprintf( f, "// termination condition\n" );
  fprintf( f, "struct sembuf fin_pls[] = { {%d,0,0} };\n", m-1);
  fprintf( f, "int fin_pls_num = 1;\n\n" );
  
  fprintf( f, "// end of PTN in the form of LSVS\n" );
  ...
  free(b); free(d); free(t_neib_count);
...
void WritePIN_map_h( FILE * f )
{
  int cp,p,pin,ip=0,op=0;
  char *lab;
  struct cpl_struct *ipl, *opl;
  
  ipl=malloc(mi*sizeof(struct cpl_struct));
  opl=malloc(mo*sizeof(struct cpl_struct));
  if(ipl==NULL || opl==NULL)
  {
  	printf("*** error: no memory for ipl or opl\n");
  	exit(-2);
  }
...
//for(cp=0; cp<mi; cp++) { printf("input p=%d,pin=%d\n", ipl[cp].p, ipl[cp].pin); }
//for(cp=0; cp<mo; cp++) { printf("input p=%d,pin=%d\n", opl[cp].p, opl[cp].pin); }

  fprintf( f, "\n//SN pin map\n\n");								// SN pin map
  fprintf( f, "#define MI %d\n#define MO %d\n\n#define DEBOUNCE_DELAY 50\n", mi, mo );	//#define MI mi
  fprintf( f, "static int in_places[MI]={");		
  for(cp=0;cp<mi;cp++) fprintf( f, "%d%s", ipl[cp].p-1, (cp<mi-1)?",":"};\n" );
  fprintf( f, "static int in_pins[MO]={");		 
  for(cp=0;cp<mi;cp++) fprintf( f, "%d%s", ipl[cp].pin, (cp<mi-1)?",":"};\n" );
  fprintf( f, "volatile static int in_pins_debounce_prev_time[MO]={" ); 
  for(cp=0;cp<mi;cp++) fprintf( f, "%d%s", 0, (cp<mi-1)?",":"};\n" );
  fprintf( f, "volatile static int in_mu[MI]={" );
  for(cp=0;cp<mi;cp++) fprintf( f, "%d%s", 0, (cp<mi-1)?",":"};\n" );
  fprintf( f, "volatile static int pi=0;\n" );
  fprintf( f, "volatile static int dbt=0;\n\n" );
  fprintf( f, "static int out_places[MO]={" );
  for(cp=0;cp<mo;cp++) fprintf( f, "%d%s", opl[cp].p-1, (cp<mo-1)?",":"};\n" );
  fprintf( f, "static int out_pins[MO]={" );
  for(cp=0;cp<mo;cp++) fprintf( f, "%d%s", opl[cp].pin, (cp<mo-1)?",":"};\n\n" ); 
//  static int out_pins[MO]={16,17,18};
  fprintf( f, "void IR_Read_Gen() {\n"
"  dbt = millis();\n"
...
"  }\n" );
for(cp=0;cp<mi;cp++) fprintf( f, "  digitalWrite(%d,LOW);\n", opl[cp].pin ); // switch of LED on next button pressed
fprintf( f, "}\n" );
  for(cp=0;cp<mi;cp++) {
  	fprintf( f, "void IR_Read_%d() { pi=%d; IR_Read_Gen(); }\n", cp, cp );
  }
  fprintf( f, "\nvoid Init_pins_IR()\n"
"{\n"
"  int i;\n"
...
"  }\n" );
  for(cp=0;cp<mi;cp++) {
  	fprintf( f, "  attachInterrupt(digitalPinToInterrupt(in_pins[%d]), IR_Read_%d, RISING);\n", cp, cp );
  }
  fprintf( f, "}\n\n" );
  fprintf( f, "void ReadIn()\n"
"  {\n"
"  int pi;\n"
"  for(pi=0;pi<MI;pi++)\n"
"    if(in_mu[pi]>0)\n"
...
int NDRtoSN( char * NetFileName, char * SNFileName, int write_name_tables, int matr )
{
 char nFileName[ FILENAMELEN+1 ];
 FILE * NetFile, * SNFile, * nFile, * OutFile, * PINMFile;
 int format;
 int z;
 ...
 if( strcmp( SNFileName, "-" )==0 ) SNFile = stdout;
   else 
   {
   	 SNFile = fopen( SNFileName, "w" );
   }
 if( SNFile == NULL ) {printf( "*** error open file %s\n", SNFileName );exit(2);}
 ...
 pl = (int*) calloc( max_pl, sizeof(int) ); ml=0; mi=0; mo=0;
 plp = (int*) calloc( max_pl, sizeof(int) ); 
 ...
 switch( matr ) // *** added for LSVS
 {
   case 0: WriteLSN( SNFile ); break;
   case 1: WriteSN_matr_h( SNFile ); break; // sn.h
   case 2: WritePIN_map_h( SNFile ); break; // pin.h
   case 3: WriteLSVS_h( SNFile ); break; // tpn.h
   default: fprintf(stderr,"*** error: invalid choice %d\n", matr ); break;
 }

 // if( matr)
 // {
 // 	WriteSN_matr_h( SNFile ); // sn.h
 //	    sprintf( nFileName, "%s_pin.h", SNFileName );
 //	    PINMFile = fopen( nFileName, "w" );
 // 	if(matr>1) WritePIN_map_h( SNFile ); // pin.h
 // } 
 // else WriteLSN( SNFile );
 if( SNFile != stdout )fclose( SNFile );
 
 if(write_name_tables)
 {
   sprintf( nFileName, "%s.nmp", SNFileName );
   nFile = fopen( nFileName, "w" );
   if( nFile == NULL ) {printf( "*** error open file %s\n", nFileName );exit(2);}
   WriteNMP( nFile );
   fclose( nFile );
 
   sprintf( nFileName, "%s.nmt", SNFileName );
   nFile = fopen( nFileName, "w" );
   if( nFile == NULL ) {printf( "*** error open file %s\n", nFileName );exit(2);}
   WriteNMT( nFile );
   fclose( nFile );
...
#ifdef __MAIN__
static char Help[] =
"NDRtoX - version 2.0.2\n\n"
"action: converts .ndr file to .lsn/.hsn or C language header .h\n"
...
"-l               output SN as .lsn/.hsn\n"
"-c               output SN as .h\n" 
"-i               output SN as .h with I/O place map onto pins as .h\n"
"-v               output TPN LSVS as .h (DEFAULT)\n"
"ndr_file         input Sleptsov/Petri net in .ndr format\n"
"output_file  	  Sleptsov/Petri net in .lsn or .hsn format\n"
"                 or as C language header .h\n\n"
...
int main( int argc, char *argv[] )
{
  ...
  int i, numf=0, c_headers=0;
    ...
    for( i=1; i<argc; i++ )
    {
      ...
      else if( strcmp( argv[i], "-v" )==0 ) c_headers=3;
	  else if( strcmp( argv[i], "-i" )==0 ) c_headers=2;
    ...
    NDRtoSN( InFileName, OutFileName, 0, c_headers);
...
// @ Dmitry Zaitsev 2025 daze@acm.org
